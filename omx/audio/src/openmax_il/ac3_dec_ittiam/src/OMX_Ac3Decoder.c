...
#define AC3_DEC_ROLE "audio_decoder.ac3"
...
OMX_ERRORTYPE OMX_ComponentInit (OMX_HANDLETYPE hComp)
{
    ...
    AC3D_OMX_MALLOC(pHandle->pComponentPrivate,AC3DEC_COMPONENT_PRIVATE);

    pComponentPrivate = pHandle->pComponentPrivate;
    pComponentPrivate->pHandle = pHandle;
    OMX_DBG_INIT(pComponentPrivate->dbg, "OMX_DBG_AC3DEC");

#ifdef __PERF_INSTRUMENTATION__
    pComponentPrivate->pPERF = PERF_Create(PERF_FOURCC('A','C','3','D'),
                                           PERF_ModuleLLMM |
    ...
    AC3D_OMX_MALLOC(pCompPort, AUDIODEC_PORT_TYPE);
    pComponentPrivate->pCompPort[INPUT_PORT_AC3DEC] =  pCompPort;

    AC3D_OMX_MALLOC(pCompPort, AUDIODEC_PORT_TYPE);
    pComponentPrivate->pCompPort[OUTPUT_PORT_AC3DEC] = pCompPort;

    AC3D_OMX_MALLOC(pTemp, AC3DEC_BUFFERLIST);
    ...
    for (i=0; i < MAX_NUM_OF_BUFS_AC3DEC; i++)
    {
    ...
    AC3D_OMX_MALLOC(pComponentPrivate->sPortParam, OMX_PORT_PARAM_TYPE);
    OMX_CONF_INIT_STRUCT(pComponentPrivate->sPortParam, OMX_PORT_PARAM_TYPE);
    AC3D_OMX_MALLOC(pComponentPrivate->pPriorityMgmt, OMX_PRIORITYMGMTTYPE);
    OMX_CONF_INIT_STRUCT(pComponentPrivate->pPriorityMgmt, OMX_PRIORITYMGMTTYPE);
    pComponentPrivate->sPortParam->nPorts = NUM_OF_PORTS_AC3DEC;
    pComponentPrivate->sPortParam->nStartPortNumber = 0x0;

    pComponentPrivate->ac3Params = NULL;
    pComponentPrivate->pcmParams = NULL;
    AC3D_OMX_MALLOC(ac3_ip,OMX_AUDIO_PARAM_AC3TYPE);
    AC3D_OMX_MALLOC(ac3_op,OMX_AUDIO_PARAM_PCMMODETYPE);
    pComponentPrivate->ac3Params = ac3_ip;
    pComponentPrivate->pcmParams = ac3_op;
...
// Jerky Related change [
    pComponentPrivate->framemode          = 1; //Shashank
// Jerky Related change ]
    pComponentPrivate->iStereoMode        = 2; //Shashank
    ...
    AC3DEC_OMX_MALLOC_SIZE(pComponentPrivate->sDeviceString,(100*sizeof(OMX_STRING)),OMX_STRING);
    strcpy((char*)pComponentPrivate->sDeviceString,"/eteedn:i0:o0/codec\0");

    /* initialize role name */
    AC3D_OMX_MALLOC(pComponentPrivate->componentRole,OMX_PARAM_COMPONENTROLETYPE);
    strcpy((char*)pComponentPrivate->componentRole->cRole, AC3_DEC_ROLE);

    AC3D_OMX_MALLOC(pPortDef_ip, OMX_PARAM_PORTDEFINITIONTYPE);
    AC3D_OMX_MALLOC(pPortDef_op, OMX_PARAM_PORTDEFINITIONTYPE);

    pComponentPrivate->pPortDef[INPUT_PORT_AC3DEC] = pPortDef_ip;
    pComponentPrivate->pPortDef[OUTPUT_PORT_AC3DEC] = pPortDef_op;
...
//-- #endif
    ...
    pPortDef_ip->nPortIndex                         = INPUT_PORT_AC3DEC;
    pPortDef_ip->eDir                               = OMX_DirInput;
    pPortDef_ip->nBufferCountActual                 = AC3D_NUM_INPUT_BUFFERS;
    pPortDef_ip->nBufferCountMin                    = AC3D_NUM_INPUT_BUFFERS;
    pPortDef_ip->nBufferSize                        = AC3D_INPUT_BUFFER_SIZE;
    pPortDef_ip->bEnabled                           = OMX_TRUE;
    pPortDef_ip->bPopulated                         = OMX_FALSE;
    pPortDef_ip->eDomain                            = OMX_PortDomainAudio;
    pPortDef_ip->format.audio.eEncoding             = OMX_AUDIO_CodingAC3;
    ...
    pPortDef_op->nPortIndex                         = OUTPUT_PORT_AC3DEC;
    pPortDef_op->eDir                               = OMX_DirOutput;
    pPortDef_op->nBufferCountMin                    = AC3D_NUM_OUTPUT_BUFFERS;
    pPortDef_op->nBufferCountActual                 = AC3D_NUM_OUTPUT_BUFFERS;
    pPortDef_op->nBufferSize                        = MIN_AC3D_OUTPUT_BUFFER_SIZE;
    ...
    AC3D_OMX_MALLOC(pComponentPrivate->pCompPort[INPUT_PORT_AC3DEC]->pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);
    AC3D_OMX_MALLOC(pComponentPrivate->pCompPort[OUTPUT_PORT_AC3DEC]->pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);
    OMX_CONF_INIT_STRUCT(pComponentPrivate->pCompPort[INPUT_PORT_AC3DEC]->pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);
    OMX_CONF_INIT_STRUCT(pComponentPrivate->pCompPort[OUTPUT_PORT_AC3DEC]->pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);

    pComponentPrivate->bPreempted = OMX_FALSE;

    /* Set input port format defaults */
    pPortFormat = pComponentPrivate->pCompPort[INPUT_PORT_AC3DEC]->pPortFormat;
    OMX_CONF_INIT_STRUCT(pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);
    pPortFormat->nPortIndex         = INPUT_PORT_AC3DEC;
    pPortFormat->nIndex             = OMX_IndexParamAudioAc3;
    pPortFormat->eEncoding          = OMX_AUDIO_CodingAC3;

    /* Set output port format defaults */
    pPortFormat = pComponentPrivate->pCompPort[OUTPUT_PORT_AC3DEC]->pPortFormat;
    OMX_CONF_INIT_STRUCT(pPortFormat, OMX_AUDIO_PARAM_PORTFORMATTYPE);
    pPortFormat->nPortIndex         = OUTPUT_PORT_AC3DEC;
    pPortFormat->nIndex             = OMX_IndexParamAudioPcm;
    pPortFormat->eEncoding          = OMX_AUDIO_CodingPCM;

    /* AC3 format defaults */
    OMX_CONF_INIT_STRUCT(ac3_ip, OMX_AUDIO_PARAM_AC3TYPE);
    ac3_ip->nPortIndex = INPUT_PORT_AC3DEC;

//Shashank [
    pComponentPrivate->bShouldSendEOStoApp = OMX_FALSE;
//Shashank ]

    ac3_ip->nSampleRate = AC3D_SAMPLING_FREQUENCY;
    ac3_ip->nChannels = 2;//6;Downmixed for LG
    ac3_ip->eChannelMode = 2;//7;Downmixed for LG
    ac3_ip->lfeflag = 0;//1;Downmixed for LG
    ac3_ip->i_output_pcm_fmt = 16;
    ac3_ip->i_k_capable_mode = 3;
    ac3_ip->i_comp_mode = 2;
    ac3_ip->i_pcm_scale_fac = (1<<15);
    ac3_ip->i_stereo_mode = 2;
    ac3_ip->i_dualmono_mode = 0;
    ac3_ip->i_dyn_rng_scale_hi = (1<<15);
    ac3_ip->i_dyn_rng_scale_low = (1<<15);
    ...
    /* PCM format defaults */
    OMX_CONF_INIT_STRUCT(ac3_op, OMX_AUDIO_PARAM_PCMMODETYPE);

    ac3_op->eNumData      = OMX_NumericalDataSigned;
    ac3_op->ePCMMode      = OMX_AUDIO_PCMModeLinear;
    ac3_op->nPortIndex    = OUTPUT_PORT_AC3DEC;
    ac3_op->nBitPerSample = 16;
    ac3_op->nChannels     = 2;//6;Downmixed for LG
    ac3_op->nSamplingRate = AC3D_SAMPLING_FREQUENCY;
    ac3_op->bInterleaved  = OMX_TRUE;
...
#endif
   eError = Ac3Dec_StartCompThread(pHandle);
    if (eError != OMX_ErrorNone) {
...
#ifdef __PERF_INSTRUMENTATION__
    PERF_ThreadCreated(pComponentPrivate->pPERF, pComponentPrivate->ComponentThread,PERF_FOURCC('A','C','3','T'));
#endif
 ...
 EXIT:
    if(OMX_ErrorNone != eError)
    {
        ...
        AC3DEC_OMX_FREE(pPortDef_ip);
        AC3DEC_OMX_FREE(pPortDef_op);
        AC3DEC_OMX_FREE(ac3_ip);
        AC3DEC_OMX_FREE(ac3_op);
        AC3DEC_OMX_FREE(pTemp);
    }
                                  ...
                                  OMX_U32 nParam,OMX_PTR pCmdData)
{
    ...
    AC3DEC_COMPONENT_PRIVATE *pCompPrivate = NULL;

    AC3DEC_OMX_CONF_CHECK_CMD(pHandle,1,1)
        pCompPrivate = (AC3DEC_COMPONENT_PRIVATE *)pHandle->pComponentPrivate;
    ...
    if(pCompPrivate->curState == OMX_StateInvalid)
    {
       OMX_ERROR4(pCompPrivate->dbg, "%d ::Error returned from the Component\n",
                       __LINE__);
       AC3DEC_OMX_ERROR_EXIT(eError, OMX_ErrorInvalidState,"OMX_ErrorInvalidState");
    }
#endif
#ifdef __PERF_INSTRUMENTATION__
    ...
    case OMX_CommandFlush:
        ...
        {
            AC3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadPortIndex,"OMX_ErrorBadPortIndex");
        }
        break;
    case OMX_CommandPortDisable:
    	{
        OMX_PRDSP2(pCompPrivate->dbg, "%d :: AC3DEC: Entered switch - Command Port Disable \n",__LINE__);
        //set here to 1
        pCompPrivate->port_disable = 1;
	    }
        break;
    case OMX_CommandPortEnable:
        OMX_PRDSP2(pCompPrivate->dbg, "%d :: AC3DEC: Entered switch - Command Port Enable \n",__LINE__);
        break;
    case OMX_CommandMarkBuffer:
        OMX_PRDSP2(pCompPrivate->dbg, "%d :: AC3DEC: Entered switch - Command Mark Buffer\n",__LINE__);
        if(nParam > 0) {
            AC3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadPortIndex,"OMX_ErrorBadPortIndex");
        }
        break;
    ...
    default:
        OMX_ERROR4(pCompPrivate->dbg, "%d :: AC3DEC: Entered switch - Default\n",__LINE__);
    ...
    if (nRet == -1)
    {
        ...
        AC3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorInsufficientResources,"write failed: OMX_ErrorInsufficientResources");
    }
    ...
    if (nRet == -1)
    {
        AC3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorInsufficientResources,"OMX_ErrorInsufficientResources");
    }
                                   ...
                                   OMX_PTR ComponentParameterStructure)
{
    ...
    AC3DEC_OMX_CONF_CHECK_CMD(hComp,1,1)
        pComponentPrivate = (AC3DEC_COMPONENT_PRIVATE *)(((OMX_COMPONENTTYPE*)hComp)->pComponentPrivate);
    AC3DEC_OMX_CONF_CHECK_CMD(pComponentPrivate, ComponentParameterStructure, 1)
    ...
    if(pComponentPrivate->curState == OMX_StateInvalid) {
        OMX_ERROR4(pComponentPrivate->dbg, "%d ::Error returned from the Component\n",
                       __LINE__);
        AC3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorIncorrectStateOperation,"write failed: OMX_ErrorIncorrectStateOperation");
    }
#endif
    ...
    OMX_PRDSP2(pComponentPrivate->dbg, "%d :: Entering OMX_IndexParamPortDefinition\n", __LINE__);
        if(((OMX_PARAM_PORTDEFINITIONTYPE *)(ComponentParameterStructure))->nPortIndex ==
           pComponentPrivate->pPortDef[INPUT_PORT_AC3DEC]->nPortIndex) {
            memcpy(ComponentParameterStructure, pComponentPrivate->pPortDef[INPUT_PORT_AC3DEC], sizeof(OMX_PARAM_PORTDEFINITIONTYPE));
        } else if(((OMX_PARAM_PORTDEFINITIONTYPE *)(ComponentParameterStructure))->nPortIndex ==
                  pComponentPrivate->pPortDef[OUTPUT_PORT_AC3DEC]->nPortIndex) {
            memcpy(ComponentParameterStructure, pComponentPrivate->pPortDef[OUTPUT_PORT_AC3DEC], sizeof(OMX_PARAM_PORTDEFINITIONTYPE));
        } else {
        ...
        if(((OMX_AUDIO_PARAM_PORTFORMATTYPE *)(ComponentParameterStructure))->nPortIndex ==
           pComponentPrivate->pPortDef[INPUT_PORT_AC3DEC]->nPortIndex) {
            if(((OMX_AUDIO_PARAM_PORTFORMATTYPE *)(ComponentParameterStructure))->nIndex >
               pComponentPrivate->pCompPort[INPUT_PORT_AC3DEC]->pPortFormat->nPortIndex) {
                eError = OMX_ErrorNoMore;
            }
            else {
                memcpy(ComponentParameterStructure, pComponentPrivate->pCompPort[INPUT_PORT_AC3DEC]->pPortFormat, sizeof(OMX_AUDIO_PARAM_PORTFORMATTYPE));
            }
        }
        else if(((OMX_AUDIO_PARAM_PORTFORMATTYPE *)(ComponentParameterStructure))->nPortIndex ==
                pComponentPrivate->pPortDef[OUTPUT_PORT_AC3DEC]->nPortIndex){
            if(((OMX_AUDIO_PARAM_PORTFORMATTYPE *)(ComponentParameterStructure))->nIndex >
               pComponentPrivate->pCompPort[OUTPUT_PORT_AC3DEC]->pPortFormat->nPortIndex) {
                eError = OMX_ErrorNoMore;
            }
            else {
                memcpy(ComponentParameterStructure, pComponentPrivate->pCompPort[OUTPUT_PORT_AC3DEC]->pPortFormat, sizeof(OMX_AUDIO_PARAM_PORTFORMATTYPE));
            }
        }
        else {
            OMX_ERROR2(pComponentPrivate->dbg, "%d :: OMX_ErrorBadPortIndex from GetParameter \n",__LINE__);
    ...
    OMX_PRDSP2(pComponentPrivate->dbg, "%d :: Entering OMX_IndexParamAudioAc3\n", __LINE__);
        if(((OMX_AUDIO_PARAM_AC3TYPE *)(ComponentParameterStructure))->nPortIndex ==
           pComponentPrivate->ac3Params->nPortIndex) {
            memcpy(ComponentParameterStructure, pComponentPrivate->ac3Params, sizeof(OMX_AUDIO_PARAM_AC3TYPE));
                                   ...
                                   OMX_PTR pCompParam)
{
    ...
    OMX_AUDIO_PARAM_AC3TYPE *pCompAc3Param = NULL;
    ...
    AC3DEC_OMX_CONF_CHECK_CMD(pComponentPrivate, pCompParam, 1)

    if (pComponentPrivate->curState != OMX_StateLoaded) {
        AC3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorIncorrectStateOperation,"OMX_ErrorIncorrectStateOperation");
    }
    ...
    case OMX_IndexParamAudioPortFormat:
        {
            ...
            if ( pComponentParam->nPortIndex == pComponentPrivate->pCompPort[INPUT_PORT_AC3DEC]->pPortFormat->nPortIndex ) {
                memcpy(pComponentPrivate->pCompPort[INPUT_PORT_AC3DEC]->pPortFormat, pComponentParam, sizeof(OMX_AUDIO_PARAM_PORTFORMATTYPE));
            } else if ( pComponentParam->nPortIndex == pComponentPrivate->pCompPort[OUTPUT_PORT_AC3DEC]->pPortFormat->nPortIndex ) {
                memcpy(pComponentPrivate->pCompPort[OUTPUT_PORT_AC3DEC]->pPortFormat, pComponentParam, sizeof(OMX_AUDIO_PARAM_PORTFORMATTYPE));
            } else {
    ...
    case OMX_IndexParamAudioAc3:
        {
            pCompAc3Param = (OMX_AUDIO_PARAM_AC3TYPE *)pCompParam;
            if(pCompAc3Param->nPortIndex == 0)
            { /* 0 means Input port */
                memcpy(((AC3DEC_COMPONENT_PRIVATE*)
                        pHandle->pComponentPrivate)->ac3Params, pCompAc3Param, sizeof(OMX_AUDIO_PARAM_AC3TYPE));
    ...
    case OMX_IndexParamPortDefinition:
        {
            ...
            if (pComponentParamPort->nPortIndex == 0) {
                ...
                if (pComponentParamPort->format.audio.eEncoding != OMX_AUDIO_CodingAC3) {
            ...
            if(((OMX_PARAM_PORTDEFINITIONTYPE *)(pCompParam))->nPortIndex ==
               pComponentPrivate->pPortDef[INPUT_PORT_AC3DEC]->nPortIndex) {
                OMX_PRINT2(pComponentPrivate->dbg, "%d :: SetParameter OMX_IndexParamPortDefinition \n",__LINE__);
                memcpy(pComponentPrivate->pPortDef[INPUT_PORT_AC3DEC], pCompParam,sizeof(OMX_PARAM_PORTDEFINITIONTYPE));
            }
            else if(((OMX_PARAM_PORTDEFINITIONTYPE *)(pCompParam))->nPortIndex ==
                    pComponentPrivate->pPortDef[OUTPUT_PORT_AC3DEC]->nPortIndex) {
                OMX_PRINT2(pComponentPrivate->dbg, "%d :: SetParameter OMX_IndexParamPortDefinition \n",__LINE__);
                memcpy(pComponentPrivate->pPortDef[OUTPUT_PORT_AC3DEC], pCompParam, sizeof(OMX_PARAM_PORTDEFINITIONTYPE));
            }
            else {
                OMX_ERROR2(pComponentPrivate->dbg, "%d :: OMX_ErrorBadPortIndex from SetParameter",__LINE__);
                eError = OMX_ErrorBadPortIndex;
                                ...
                                OMX_PTR ComponentConfigStructure)
{
	...
	pComponentPrivate = (AC3DEC_COMPONENT_PRIVATE *)pHandle->pComponentPrivate;
    ...
    switch (nConfigIndex)
    {
    case  OMX_IndexCustomAc3DecDataPath:
        pComponentPrivate->framemode = *((int *)ComponentConfigStructure);
        break;

    case  OMX_IndexCustomAc3DecStereoMode:
	        pComponentPrivate->iStereoMode = *((int *)ComponentConfigStructure);
            ALOGD("Dolby effect is set to %d", pComponentPrivate->iStereoMode);
        break;
                                   ...
                                   OMX_PTR pAppData)
{
    ...
    AC3DEC_OMX_CONF_CHECK_CMD(pComponentPrivate,1,1)

    AC3DEC_OMX_CONF_CHECK_CMD(pCallBacks, pCallBacks->EventHandler, pCallBacks->EmptyBufferDone)
    AC3DEC_OMX_CONF_CHECK_CMD(pCallBacks->FillBufferDone, 1, 1)
                                          ...
                                          OMX_UUIDTYPE* pComponentUUID)
{
    OMX_ERRORTYPE eError = OMX_ErrorNone;
    OMX_COMPONENTTYPE *pHandle = (OMX_COMPONENTTYPE*) hComp;
    AC3DEC_COMPONENT_PRIVATE *pComponentPrivate = (AC3DEC_COMPONENT_PRIVATE *) pHandle->pComponentPrivate;
                                ...
                                OMX_PTR ComponentConfigStructure)
{
    ...
    if (nConfigIndex ==  OMX_IndexParamAudioAc3)
    {
        OMX_PRDSP2(pComponentPrivate->dbg, "%d :: GetConfig OMX_IndexParamAudioAc3 \n",__LINE__);
        if(((OMX_AUDIO_PARAM_AC3TYPE *)(ComponentConfigStructure))->nPortIndex ==
           pComponentPrivate->ac3Params->nPortIndex)
        {
            memcpy(ComponentConfigStructure, pComponentPrivate->ac3Params, sizeof(OMX_AUDIO_PARAM_AC3TYPE));
        }
        else if(((OMX_AUDIO_PARAM_PCMMODETYPE *)(ComponentConfigStructure))->nPortIndex ==
                pComponentPrivate->pcmParams->nPortIndex)
        {
...
static OMX_ERRORTYPE GetState (OMX_HANDLETYPE pComponent, OMX_STATETYPE* pState)
{
    ...
    AC3DEC_OMX_CONF_CHECK_CMD(pHandle,1,1);
        ...
        OMXDBG_PRINT(stderr, STATE, 2, 0, "%d :: In GetState\n", __LINE__);
            *pState =  ((AC3DEC_COMPONENT_PRIVATE*)
                                      ...
                                      OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    int ret=0;
    pPortDef = ((AC3DEC_COMPONENT_PRIVATE*)pComponentPrivate)->pPortDef[INPUT_PORT_AC3DEC];
    ...
    if (pBuffer == NULL) {
        AC3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadParameter,"OMX_ErrorBadParameter");
    }

    if (pBuffer->nSize != sizeof(OMX_BUFFERHEADERTYPE)) {
        AC3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadParameter,"Bad Size");
    }

    if (pBuffer->nInputPortIndex != INPUT_PORT_AC3DEC) {
        AC3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadPortIndex,"OMX_ErrorBadPortIndex");
    }


    if (pBuffer->nVersion.nVersion != pComponentPrivate->nVersion) {
        AC3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorVersionMismatch,"OMX_ErrorVersionMismatch");
    }
    ...
    OMX_PRCOMM2(pComponentPrivate->dbg, "%d:: Writing input buffer[%d] into pipe:%d \n",__LINE__,pComponentPrivate->nUnhandledEmptyThisBuffers, pBuffer->nFilledLen);

    ret = write (pComponentPrivate->dataPipeInputBuf[1], &pBuffer,sizeof(OMX_BUFFERHEADERTYPE*));
    if (ret == -1)
    {
        AC3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorHardware,"write failed: OMX_ErrorHardware");
    }


 EXIT:
	OMX_PRCOMM2(pComponentPrivate->dbg, "%d:: Exiting from EmptyThisBuffer \n",__LINE__);
    return eError;
}
...
/**
  ...
  *
  * @retval OMX_NoError              Success, ready to roll
  *         OMX_Error_BadParameter   The INPUT_PORT_AC3DEC parameter pointer is null
  **/
/*-------------------------------------------------------------------*/
                                     ...
                                     OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    pPortDef = ((AC3DEC_COMPONENT_PRIVATE*)pComponentPrivate)->pPortDef[OUTPUT_PORT_AC3DEC];
    ...
    if (pBuffer == NULL) {
        AC3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadParameter,"write failed: OMX_ErrorBadParameter");
    }

    if (pBuffer->nSize != sizeof(OMX_BUFFERHEADERTYPE)) {
        OMX_PRBUFFER2(pComponentPrivate->dbg, "%d :: EmptyThisBuffer: Bad Size\n",__LINE__);
        AC3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadParameter,"write failed: Bad Size");
    }

    if (pBuffer->nOutputPortIndex != OUTPUT_PORT_AC3DEC) {
        OMX_PRBUFFER2(pComponentPrivate->dbg, "%d :: EmptyThisBuffer: BadPortIndex\n",__LINE__);
        AC3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadPortIndex,"write failed: BadPortIndex");
    }
    ...
    if (pBuffer->nVersion.nVersion != pComponentPrivate->nVersion) {
        ...
        OMX_PRBUFFER2(pComponentPrivate->dbg, "%d::pBuffer->nVersion.nVersion:%lu\n",__LINE__,pBuffer->nVersion.nVersion);
        OMX_PRBUFFER2(pComponentPrivate->dbg, "%d::pComponentPrivate->nVersion:%lu\n",__LINE__,pComponentPrivate->nVersion);
        AC3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorVersionMismatch,"write failed: OMX_ErrorVersionMismatch");
    }
    if(pComponentPrivate->curState != OMX_StateExecuting && pComponentPrivate->curState != OMX_StatePause) {
        AC3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorIncorrectStateOperation,"write failed: OMX_ErrorIncorrectStateOperation");
    }
...
static OMX_ERRORTYPE ComponentDeInit(OMX_HANDLETYPE pHandle)
{
    ...
    AC3DEC_COMPONENT_PRIVATE *pComponentPrivate = NULL;
   ...
   // OMX_PRCOMM2(dbg, ":: ComponentDeinit: Entered \n");
    AC3DEC_OMX_CONF_CHECK_CMD(pComponent,1,1)
    pComponentPrivate = (AC3DEC_COMPONENT_PRIVATE *)pComponent->pComponentPrivate;
...
#ifdef RESOURCE_AC3_MANAGER_ENABLED
    eError = RMProxy_NewSendCommand(pHandle, RMProxy_FreeResource,OMX_AC3_Decoder_COMPONENT, 0, 3456, NULL);
    if (eError != OMX_ErrorNone) {
    ...
    eError1 = AC3DEC_FreeCompResources(pHandle);
    ...
    AC3DEC_OMX_FREE(pComponentPrivate->sDeviceString);
    ...
    OMX_DBG_CLOSE(dbg);
    AC3DEC_OMX_FREE(pComponentPrivate);
                   ...
                   OMX_IN OMX_U32 nSizeBytes)
{
    ...
    AC3DEC_OMX_CONF_CHECK_CMD(hComponent,1,1);
    pComponentPrivate = (AC3DEC_COMPONENT_PRIVATE *)(((OMX_COMPONENTTYPE*)hComponent)->pComponentPrivate);
    ...
    pPortDef = ((AC3DEC_COMPONENT_PRIVATE*)pComponentPrivate)->pPortDef[nPortIndex];

    AC3DEC_OMX_CONF_CHECK_CMD(pPortDef, 1, 1);
    if (!pPortDef->bEnabled) {
    ...
    AC3D_OMX_MALLOC(pBufferHeader, OMX_BUFFERHEADERTYPE);
    memset((pBufferHeader), 0x0, sizeof(OMX_BUFFERHEADERTYPE));

    AC3DEC_OMX_MALLOC_SIZE(pBufferHeader->pBuffer,(nSizeBytes + DSP_CACHE_ALIGNMENT),OMX_U8);
    OMX_PRBUFFER2(pComponentPrivate->dbg, "%d :: Malloced = %p\n",__LINE__,pBufferHeader->pBuffer);
    pBufferHeader->nVersion.nVersion = AC3DEC_BUFHEADER_VERSION;
    ...
    if (nPortIndex == INPUT_PORT_AC3DEC)
    {
    ...
    } else if (nPortIndex == OUTPUT_PORT_AC3DEC) {
    ...
    if((pComponentPrivate->pPortDef[OUTPUT_PORT_AC3DEC]->bPopulated == pComponentPrivate->pPortDef[OUTPUT_PORT_AC3DEC]->bEnabled)&&
       (pComponentPrivate->pPortDef[INPUT_PORT_AC3DEC]->bPopulated == pComponentPrivate->pPortDef[INPUT_PORT_AC3DEC]->bEnabled) &&
    ...
    pBufferHeader->pPlatformPrivate = pComponentPrivate;
    pBufferHeader->nAllocLen = nSizeBytes;
    pBufferHeader->nVersion.s.nVersionMajor = AC3DEC_MAJOR_VER;
    pBufferHeader->nVersion.s.nVersionMinor = AC3DEC_MINOR_VER;
 ...
 EXIT:
    if(OMX_ErrorNone != eError) {
        ...
        AC3DEC_OMX_FREE(pBufferHeader->pBuffer);
        AC3DEC_OMX_FREE(pBufferHeader);
    }
                                ...
                                OMX_IN  OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    if (inputIndex != -1)
    {
        ...
        AC3DEC_OMX_FREE(pComponentPrivate->pInputBufferList->pBufHdr[inputIndex]);
        pComponentPrivate->pInputBufferList->pBufHdr[inputIndex] = NULL;
        pComponentPrivate->pInputBufferList->numBuffers--;

        if (pComponentPrivate->pInputBufferList->numBuffers <
            pComponentPrivate->pPortDef[INPUT_PORT_AC3DEC]->nBufferCountMin)
        {

            pComponentPrivate->pPortDef[INPUT_PORT_AC3DEC]->bPopulated = OMX_FALSE;
        }
        ...
        if(pComponentPrivate->pPortDef[INPUT_PORT_AC3DEC]->bEnabled &&
          (pComponentPrivate->port_disable == 0) && //(if the var == 0)
            ...
            pComponentPrivate->curState == OMX_StatePause))
        {
			...
			OMX_ERROR4(pComponentPrivate->dbg, "%d: port disable: %d \n",__LINE__,pComponentPrivate->port_disable);
    ...
    else if (outputIndex != -1)
    {
        ...
        AC3DEC_OMX_FREE(pComponentPrivate->pOutputBufferList->pBufHdr[outputIndex]->pOutputPortPrivate);
        AC3DEC_OMX_FREE(pComponentPrivate->pOutputBufferList->pBufHdr[outputIndex]);
        ...
        OMX_PRCOMM2(pComponentPrivate->dbg, "pComponentPrivate->pPortDef[OUTPUT_PORT_AC3DEC]->nBufferCountMin = %lu\n",pComponentPrivate->pPortDef[OUTPUT_PORT_AC3DEC]->nBufferCountMin);
        if (pComponentPrivate->pOutputBufferList->numBuffers <
            pComponentPrivate->pPortDef[OUTPUT_PORT_AC3DEC]->nBufferCountMin)
        {

            pComponentPrivate->pPortDef[OUTPUT_PORT_AC3DEC]->bPopulated = OMX_FALSE;
        }
        ...
        if(pComponentPrivate->pPortDef[OUTPUT_PORT_AC3DEC]->bEnabled &&
        	(pComponentPrivate->port_disable == 0) && //(if the var == 0)
           pComponentPrivate->bLoadedCommandPending == OMX_FALSE &&
           !pComponentPrivate->reconfigOutputPort &&
           (pComponentPrivate->curState == OMX_StateIdle ||
            ...
            OMX_IN OMX_U8* pBuffer)
{
    ...
    pPortDef = ((AC3DEC_COMPONENT_PRIVATE*)
...
//
    if(!pPortDef->bEnabled) {
        AC3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorIncorrectStateOperation,
                              "Port is Disabled: OMX_ErrorIncorrectStateOperation");
    }

    if(pPortDef->bPopulated) {
        AC3DEC_OMX_ERROR_EXIT(eError,OMX_ErrorBadParameter,
    ...
    memset((pBufferHeader), 0x0, sizeof(OMX_BUFFERHEADERTYPE));
    if (nPortIndex == OUTPUT_PORT_AC3DEC) {
        pBufferHeader->nInputPortIndex = -1;
        pBufferHeader->nOutputPortIndex = nPortIndex;
            ...
            OMX_OUT OMX_INDEXTYPE* pIndexType)
{
    ...
    if (!(strcmp(cParameterName,"OMX.ITTIAM.index.config.ac3decHeaderInfo"))) {
        *pIndexType = OMX_IndexCustomAc3DecHeaderInfoConfig;
        OMXDBG_PRINT(stderr, DSP, 2, 0, "OMX_IndexCustomAc3DecHeaderInfoConfig\n");
    }
    else if(!(strcmp(cParameterName,"OMX.ITTIAM.index.config.ac3decstreamIDinfo"))){
        *pIndexType = OMX_IndexCustomAc3DecStreamIDConfig;
    }
    else if(!(strcmp(cParameterName,"OMX.ITTIAM.index.config.ac3dec.datapath"))) {
        *pIndexType = OMX_IndexCustomAc3DecDataPath;
    }
    else if(!(strcmp(cParameterName,"OMX.ITTIAM.index.config.ac3dec.stereomode"))) {
        *pIndexType = OMX_IndexCustomAc3DecStereoMode;
    }
    else if(!(strcmp(cParameterName,"OMX.ITTIAM.AC3.Decode.Debug"))) {
    *pIndexType = OMX_IndexCustomDebug;
    }
    else if(!(strcmp(cParameterName,"OMX.ITTIAM.index.config.ac3.paramAudio"))) {
	    *pIndexType = OMX_IndexParamAudioAc3;
    }
    else if(!(strcmp(cParameterName,"OMX.ITTIAM.index.config.ac3.codingType"))) {
	    *pIndexType = OMX_IndexCustomAc3CodingType;
	}
    else {
        eError = OMX_ErrorBadParameter;

...
#define INPUT_MP3DEC_BUFFER_SIZE 2048*4
...
#endif
OMX_STRING strMp3Decoder = "OMX.ITTIAM.MP3.decode";
...
{
	APP_DPRINT("%d:: APP: Fill Buffer Done, outputportreconfig :%d \n",__LINE__, outputportreconfig);
...
int main(int argc, char* argv[])
{
    ...
    OMX_CALLBACKTYPE Mp3CaBa = {(void *)EventHandler,(void*) EmptyBufferDone,
                                                     (void*)FillBufferDone};
...
int Mp3dec_fdread;
	...
	APP_IPRINT("------------------------------------------------------------\n");
    APP_IPRINT("This is Main Thread In MPEG MP3 DECODER Test Application:\n");
  ...
  APP_IPRINT("Line %d\n",__LINE__);
    ...
    if(  (argc < 8) || (argc > 9)  )
	{
        ...
        APP_EPRINT("Example: Mp3Decoder_Test ip.mp3 op.pcm 1 5 16 1 1 <ROBUST>\n");
        goto EXIT;
    }
    ...
    switch (atoi(argv[4]))
	{
		...
		case 4:
			...
			if( (argc == 9) )
			{
				...
				{
					APP_IPRINT("%d :: APP: MP3: 100 Iterations - ROBUSTNESS Test mode\n",__LINE__);
					testcnt = 100;
				}
			}
			else
	...
	APP_DPRINT("%d :: APP: MP3 Decoding Test --- first create file handle\n",__LINE__);
	APP_DPRINT("%d :: APP: MP3 Decoding Test --- fIn = [%p]\n", __LINE__, fIn);
	APP_DPRINT("%d :: APP: MP3 Decoding Test --- fOut = [%p]\n",__LINE__, fOut);
...
/*----------------------------------------------
 ...
 ----------------------------------------------*/
    jj=0;
    APP_IPRINT("%d :: APP: MP3 DEC Test --- will call [%d] time for decoder\n",__LINE__, jj+1);
    for(jj=0; jj<testcnt1; jj++)
	{
...
#ifdef DSP_RENDERING_ON
		if((Mp3dec_fdwrite=open(FIFO1,O_WRONLY))<0) {
	    ...
	    if((Mp3dec_fdread=open(FIFO2,O_RDONLY))<0) {
...
#ifdef OMX_GETTIME
	    GT_START();
	    error = OMX_GetHandle(pHandle,strMp3Decoder,&AppData, &Mp3CaBa);
		...
		APP_IPRINT("%d :: APP: MP3 Decoding in FILE MODE\n",__LINE__);
		...
		pCompPrivateStruct->nBufferSize                       = INPUT_MP3DEC_BUFFER_SIZE;
	...
	GT_END("Set Parameter Test-SetParameter");
		...
		/* Setting MP3 params */
	    pMp3Param->nSize 					= sizeof (OMX_AUDIO_PARAM_MP3TYPE);
		pMp3Param->nVersion.s.nVersionMajor = 0xF1;
		pMp3Param->nVersion.s.nVersionMinor = 0xF2;
		pMp3Param->nPortIndex 				= INPUT_PORT;
		pMp3Param->nChannels 				= channel;
		pMp3Param->nBitRate					= 128000;
		pMp3Param->nSampleRate 				= 44100;
		pMp3Param->nAudioBandWidth 			= 0;
		pMp3Param->eChannelMode 			= OMX_AUDIO_ChannelModeStereo;
		pMp3Param->eFormat 					= OMX_AUDIO_MP3StreamFormatMP1Layer3;
	...
	GT_END("Set Parameter Test-SetParameter");
	    ...
	    for(i = 0; i < numofinbuff; i++)
		{
		   ...
		   error = OMX_AllocateBuffer(*pHandle, &pInputBufferHeader[i], 0, NULL, INPUT_MP3DEC_BUFFER_SIZE);
		   if(error != OMX_ErrorNone)
		   {
	    ...
	    for(i = 0; i < numofinbuff; i++)
		{
		   pInputBuffer[i] = (OMX_U8*)newmalloc(INPUT_MP3DEC_BUFFER_SIZE + 256);
		   ...
		   error = OMX_UseBuffer(*pHandle, &pInputBufferHeader[i], 0, NULL, INPUT_MP3DEC_BUFFER_SIZE, pInputBuffer[i]);
		   if(error != OMX_ErrorNone)
		   {
    		...
    		while(1)
    		{
       		if((error == OMX_ErrorNone) && (state != OMX_StateIdle) && (state != OMX_StateInvalid) && (!bPlayCompleted)){
				...
				switch (atoi(argv[4]))
				{
					...
					case 5:
						{
						    APP_DPRINT("%d :: APP: MP3 DECODER RUNNING UNDER FILE 2 FILE MODE \n",__LINE__);
					...
					case 6:
						{
							...
							if(nIpBuffs == 20)
							{
					                ...
					                if(error != OMX_ErrorNone)
									{
						                APP_EPRINT("APP: Warning:  hMp3Decoder->GetState has returned status %X\n", error);
						                goto EXIT;
									}
							}
...
/*----------------------------------------------
 ...
 ----------------------------------------------*/
		...
		if(error != OMX_ErrorNone)
		{
			APP_EPRINT("APP: Error:  hMp3Decoder->WaitForState reports an error %X\n", error);
			goto EXIT;
...
SHUTDOWN:
	    ...
	    if((write(Mp3dec_fdwrite, &cmd_data, sizeof(cmd_data)))<0)
	        APP_EPRINT("%d :: APP: ---send command to audio manager\n",__LINE__);

	    close(Mp3dec_fdwrite);
	    close(Mp3dec_fdread);
#endif

	    /* Free the Mp3Decoder Component */

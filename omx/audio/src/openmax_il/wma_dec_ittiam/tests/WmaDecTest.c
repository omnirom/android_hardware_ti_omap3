...
#define INPUT_WMADEC_BUFFER_SIZE 2048*4
...
#endif
OMX_STRING strWmaDecoder = "OMX.ITTIAM.WMA.decode";
...
int main(int argc, char* argv[])
{
    ...
    OMX_CALLBACKTYPE WmaCaBa = {(void *)EventHandler,(void*) EmptyBufferDone,
                                                     (void*)FillBufferDone};
    OMX_HANDLETYPE *pHandle = NULL;
    OMX_ERRORTYPE  error = OMX_ErrorNone;
    OMX_U32 AppData = 100;
    OMX_PARAM_PORTDEFINITIONTYPE* pCompPrivateStruct =NULL;
    OMX_AUDIO_PARAM_WMATYPE *pWmaParam =NULL;
...
int Wmadec_fdread;
	...
	APP_IPRINT("------------------------------------------------------------\n");
    APP_IPRINT("This is Main Thread In MPEG WMA DECODER Test Application:\n");
  ...
  APP_IPRINT("Line %d\n",__LINE__);
    ...
    if(  (argc < 8) || (argc > 9)  )
	{
        APP_EPRINT("Usage:   test: [INFILE] [OUTFILE] [FRAME_MODE:0/1] [TESTCASE] [PCM WIDTH:16/24] [IP_BUF] [OUT_BUF] [ROBUSTNESS]\n");
        APP_EPRINT("Example: WmaDecoder_Test ip.wma op.pcm 1 5 16 1 1 <ROBUST>\n");
        goto EXIT;
    }
    ...
    switch (atoi(argv[4]))
	{
		...
		case 4:
			...
			if( (argc == 9) )
			{
				if(!strcmp("ROBUST",argv[8]))
				{
					APP_IPRINT("%d :: APP: WMA: 100 Iterations - ROBUSTNESS Test mode\n",__LINE__);
					testcnt = 100;
				}
			}
			else
	...
	APP_DPRINT("%d :: APP: WMA Decoding Test --- first create file handle\n",__LINE__);
	APP_DPRINT("%d :: APP: WMA Decoding Test --- fIn = [%p]\n", __LINE__, fIn);
	APP_DPRINT("%d :: APP: WMA Decoding Test --- fOut = [%p]\n",__LINE__, fOut);
...
/*----------------------------------------------
 ...
 ----------------------------------------------*/
    jj=0;
    APP_IPRINT("%d :: APP: WMA DEC Test --- will call [%d] time for decoder\n",__LINE__, jj+1);
    for(jj=0; jj<testcnt1; jj++)
	{
...
#ifdef DSP_RENDERING_ON
		if((Wmadec_fdwrite=open(FIFO1,O_WRONLY))<0) {
	    ...
	    if((Wmadec_fdread=open(FIFO2,O_RDONLY))<0) {
...
#ifdef OMX_GETTIME
	    GT_START();
	    error = OMX_GetHandle(pHandle,strWmaDecoder,&AppData, &WmaCaBa);
	    GT_END("Call to GetHandle");
#else
	    error = TIOMX_GetHandle(pHandle,strWmaDecoder,&AppData, &WmaCaBa);
#endif
...
#if 0
        audioinfo.wmadecHeaderInfo = newmalloc(sizeof(WMADEC_HeadInfo));
        if (audioinfo.wmadecHeaderInfo == NULL)
		{
            APP_EPRINT("%d :: APP: Could not allocate audioinfo.wmadecHeaderInfo\n",__LINE__);
            goto EXIT;
		}
#endif
		...
		{
		    APP_IPRINT("%d :: APP: WMA Decoding in FILE MODE\n",__LINE__);
		}
		...
		OutputBufferSize = 8192*10;   /*Sample frec,  Bit rate , frames*/

		framemode = argv[3];
		/* getting index for  framemode */
		error = OMX_GetExtensionIndex(*pHandle, "OMX.ITTIAM.index.config.wmadec.datapath",&index);
	    if (error != OMX_ErrorNone)
		{
	...
	GT_END("Set Parameter Test-SetParameter");
		...
		pWmaParam->nPortIndex 				= INPUT_PORT;
		/*pWmaParam->nChannels 				= channel;
		pWmaParam->nBitRate					= 128000;
		pWmaParam->nSampleRate 				= 44100;
		pWmaParam->nAudioBandWidth 			= 0;
		pWmaParam->eChannelMode 			= OMX_AUDIO_ChannelModeStereo;
		pWmaParam->eFormat 					= OMX_AUDIO_WMAStreamFormatMP1Layer3;*/
	...
	GT_END("Set Parameter Test-SetParameter");
	    ...
	    for(i = 0; i < numofinbuff; i++)
		{
		   pInputBuffer[i] = (OMX_U8*)newmalloc(INPUT_WMADEC_BUFFER_SIZE + 256);
		   ...
		   error = OMX_UseBuffer(*pHandle, &pInputBufferHeader[i], 0, NULL, INPUT_WMADEC_BUFFER_SIZE, pInputBuffer[i]);
		   if(error != OMX_ErrorNone)
		   {
    		...
    		while(1)
    		{
       		if((error == OMX_ErrorNone) && (state != OMX_StateIdle) && (state != OMX_StateInvalid) && (!bPlayCompleted)){
				...
				switch (atoi(argv[4]))
				{
					...
					case 5:
						{
						    APP_DPRINT("%d :: APP: WMA DECODER RUNNING UNDER FILE 2 FILE MODE \n",__LINE__);
					...
					case 6:
						{
							...
							if(nIpBuffs == 20)
							{
					                ...
					                if(error != OMX_ErrorNone)
									{
						                APP_EPRINT("APP: Warning:  hWmaDecoder->GetState has returned status %X\n", error);
						                goto EXIT;
									}
							}
...
/*----------------------------------------------
 ...
 ----------------------------------------------*/
		...
		if(error != OMX_ErrorNone)
		{
			APP_EPRINT("APP: Error:  hWmaDecoder->WaitForState reports an error %X\n", error);
			goto EXIT;
...
SHUTDOWN:
		...
		APP_MEMPRINT("%d :: [TESTAPPFREE] %p\n",__LINE__,audioinfo.wmadecHeaderInfo);
#if 0
	    if(audioinfo.wmadecHeaderInfo != NULL)
		{
		    newfree(audioinfo.wmadecHeaderInfo);
		    audioinfo.wmadecHeaderInfo = NULL;
		}
#endif
	    ...
	    if((write(Wmadec_fdwrite, &cmd_data, sizeof(cmd_data)))<0)
	        APP_EPRINT("%d :: APP: ---send command to audio manager\n",__LINE__);

	    close(Wmadec_fdwrite);
	    close(Wmadec_fdread);
#endif

	    /* Free the WmaDecoder Component */

...
#endif
OMX_STRING strAacEncoder = "OMX.ITTIAM.AAC.encode";
                  ...
                  OMX_PTR pEventData)
{
   ...
   switch (eEvent) {
       ...
       case OMX_EventBufferFlag:
		   APP_IPRINT( "%d :: App: Component OMX_EventBufferFlag = %d\n", __LINE__,eEvent);
		   APP_IPRINT( "%d :: App: OMX_EventBufferFlag on port = %d\n", __LINE__, (int)nData1);
...
void FillBufferDone (OMX_HANDLETYPE hComponent, OMX_PTR ptr, OMX_BUFFERHEADERTYPE* pBuffer)
{
    ...
    APP_DPRINT("%d :: ------------- APP: Data in Bytes in O/P Buffer :%d\n",__LINE__, pBuffer->nFilledLen );
...
int main(int argc, char* argv[])
{
	...
	OMX_U32 ArrValidFrecHE[6]={32000,44100,48000};
...
int Aacenc_fdread;
  ...
  APP_IPRINT("Line %d\n",__LINE__);
    ...
    if(  (argc < 14) || (argc > 15)  )
	{
        APP_EPRINT("Usage:   test: [INFILE] [OUTFILE] [MONO/STEREO] [TESTCASE] [FRAME_MODE] [SFREQ] [BITRATE] [OBJECTTYPE] [FRAMES] [IP_BUF] [OUT_BUF] [BITRATE_MODE] [FILEFORMAT] [ROBUSTNESS]\n");
	...
	/*Opening INPUT FILE */
        ...
        APP_EPRINT("APP: Opening the input file %s for readonly access\n", argv[1]);
	fIn = fopen(argv[1], "r");
    if(fIn == NULL)
	{
	...
	/*Opening  OUTPUT FILE */
        APP_EPRINT("APP: Opening the output file %s for write access\n", argv[2]);
    fOut = fopen(argv[2], "w");
    if(fOut == NULL)
	{
...
/*----------------------------------------------
 ...
 ----------------------------------------------*/
    ...
    for(jj=0; jj<testcnt1; jj++)
	{
	    ...
	    audioinfo.dasfMode = 0 ;// atoi(argv[5]);
		...
		// Frame mode support
		framemode = atoi(argv[5]);
		APP_IPRINT("%d :: APP: framemode = %d\n",__LINE__,framemode);

		error = OMX_GetExtensionIndex(*pHandle, "OMX.ITTIAM.index.config.aacenc.framemode",&index);
	    if (error != OMX_ErrorNone)
		{
	...
	GT_END("Set Parameter Test-SetParameter");
		...
		iAacParam->bInterleaved 			= OMX_TRUE; /* Interleaved */

		APP_IPRINT("%d :: APP: iAacParam->bInterleaved %d\n",__LINE__,(iAacParam->bInterleaved));
	...
	GT_END("Set Parameter Test-SetParameter");
		...
		/*Ensuring Valid use of AAC */
		// For AAC: valid bitrates are : from 8 to 6 times samp freq per channel
		if ( (atoi(argv[8]) == ObjectTypeLC) && ( (atoi(argv[7])) > (atoi(argv[6])*channel*6) ) )
		{
			APP_EPRINT("%d :: App: Error: HE AAC Type supports a Maximum of 48 Kbps BitRate \n",__LINE__);
			goto EXIT;
		}
		/*Ensuring Valid use of HE */
		// For Heaac : valid bitrates are : from 8 to 3 times samp freq per channel
		if ((atoi(argv[8]) == ObjectTypeHE) && ( (atoi(argv[7])) > (atoi(argv[6])*channel*3) ) )
		{
			APP_EPRINT("%d :: App: Error: HE AAC Type supports a Maximum of 48 Kbps BitRate \n",__LINE__);
			goto EXIT;
		}
		/*Ensuring Valid use of HEv2  */
		// For Heaacv2 : valid bitrates are : from 8 to 3 times samp freq per channel
		if ( (atoi(argv[8]) == ObjectTypeHE2) && ( (atoi(argv[7])) > (atoi(argv[6])*channel*3) ) )
		{
...
#ifdef AACENCHEADERINFO_SUPPORTED
...
/*----------------------------------------------
 ...
 ----------------------------------------------*/
    		...
    		while(1) {
       		if((error == OMX_ErrorNone) && (state != OMX_StateIdle) && (state != OMX_StateInvalid) && (!bPlayCompleted)){
						...
						if(audioinfo.dasfMode == 0)
						{
		    		        ...
		    		        if(FD_ISSET(IpBuf_Pipe[0], &rfds))
							{
            		            ...
            		            if(done == 0)
								{
						            ...
						            APP_IPRINT("%d :: APP :: Input Buffer: Calling EmptyThisBuffer:[%p] with inp bytes [%d]  \n",__LINE__,pBuffer, nRead);
						...
						if(audioinfo.dasfMode == 0)
						{
					        ...
					        if( FD_ISSET(IpBuf_Pipe[0], &rfds) )
							{
					            ...
					            if(done == 0)
								{
							        ...
							        APP_DPRINT("%d :: APP :: Input buffer: Calling EmptyThisBuffer: %p\n",__LINE__,pBuffer);
							        APP_IPRINT("%d :: APP :: Input buffer: Calling EmptyThisBuffer with input bytes: %d\n",__LINE__,nRead);
							        pBuffer->nFilledLen = nRead;
					...
					case 3:
						if(audioinfo.dasfMode == 0)
						{
							...
							if(nIpBuffs == 9)
							{
								...
								APP_DPRINT("%d :: APP: Send command to change to OMX_StateExecuting\n",__LINE__);
								error = OMX_SendCommand(*pHandle,OMX_CommandStateSet, OMX_StateExecuting, NULL);
								if(error != OMX_ErrorNone)
								{
									APP_EPRINT("APP: Error from SendCommand-Executing State function\n");
								...
								APP_DPRINT("%d :: APP: Waiting for State to change to State Executing\n",__LINE__);
								error = WaitForState(*pHandle, OMX_StateExecuting);
							#ifdef OMX_GETTIME
								GT_END("Call to SendCommand <OMX_StateIdle>");
							#endif
				...
				if(FD_ISSET(OpBuf_Pipe[0], &rfds))
				{
					...
					if( (firstbuffer) && (pBuf->nFlags & OMX_BUFFERFLAG_CODECCONFIG) )
					{   // Discard first buffer - Config audio (PV)
					APP_DPRINT("%d :: App: Discard first buffer: %p \n",__LINE__,pBuf);
			...
			if (bPlayCompleted == OMX_TRUE )	/* Stop componet - just for F2F  mode*/
			{
				...
				GT_START();
			#endif
                                APP_DPRINT("%d :: APP: !!! Send Command to transition to OMX_StateIdle() !!!!! \n",__LINE__);
				error = OMX_SendCommand(*pHandle,OMX_CommandStateSet, OMX_StateIdle, NULL);
				if(error != OMX_ErrorNone)
				{
					APP_EPRINT("APP: Error from SendCommand-Idle(Stop) State function\n");
...
SHUTDOWN:
    ...
    for(r=0;r<500;r++)
	{
        if (lines[r]!=0)
        {
			 APP_IPRINT("Printing memory not deleted-");
...
OMX_BOOL ValidateParameters(OMX_U32 SampleRate, OMX_U32 numChannels, OMX_U32 BitRate, OMX_U32 ObjectType)
{
	OMX_U32 LimitsLC [22][4]={{8000 ,MONO   ,8000 ,48000 },
				   			{8000  ,STEREO ,16000 ,96000 },
							{11025 ,MONO   ,8000  ,66150 },
				   			{11025 ,STEREO ,16000 ,132300},
				   			{12000 ,MONO   ,8000  ,72000 },
				   			{12000 ,STEREO ,16000 ,144000},
				   			{16000 ,MONO   ,8000  ,96000 },
				   			{16000 ,STEREO ,16000 ,192000},
				   			{22050 ,MONO   ,8000  ,132300},
							{22050 ,STEREO ,16000 ,264600},
				   			{24000 ,MONO   ,8000  ,144000},
				   			{24000 ,STEREO ,16000 ,288000},
				   			{32000 ,MONO   ,8000  ,192000},
				   			{32000 ,STEREO ,16000 ,384000},
				   			{44100 ,MONO   ,8000  ,264600},
				  			{44100 ,STEREO ,16000 ,529200},
				   			{48000 ,MONO   ,8000  ,288000},
				   			{48000 ,STEREO ,16000 ,576000},
				   			{88200 ,MONO   ,8000  ,529200},
				   			{88200 ,STEREO ,16000 ,576000},
				   			{96000 ,MONO   ,8000  ,576000},
				   			{96000 ,STEREO ,16000 ,576000} };

	OMX_U32 LimitsHE [6][4]={{32000 ,MONO  ,8000  ,96000 },
				   			{32000 ,STEREO ,16000 ,192000},
				   			{44100 ,MONO   ,8000  ,132300},
				  			{44100 ,STEREO ,16000 ,264600},
				   			{48000 ,MONO   ,8000  ,144000},
				   			{48000 ,STEREO ,16000 ,288000} };

	OMX_U32 LimitsHE2[3][4]={{32000 ,STEREO ,8000 ,192000},
				   			{44100 ,STEREO ,12000 ,264600},
							{48000 ,STEREO ,12000 ,288000} };
